...
export function Skills({ skills }: SkillsProps) {
  return (
    <div className="flex flex-wrap justify-center items-center gap-6 mx-4 md:gap-10 lg:gap-10">
      {skills.map((skill) => (
        <HoverCard key={skill}>
          <HoverCardTrigger className="backdrop-blur-2xl text-foreground text-sm sm:text-xl border-2 border-foreground rounded-full p-2 shadow-md shadow-foreground/30 hover:scale-110 transition-all">
            {skill}
          </HoverCardTrigger>
